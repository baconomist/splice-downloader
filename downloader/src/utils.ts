...
export async function launchBrowser({ withProxy = true, optimized = true, args = [], ignoreDefaultArgs = [], headless = null, executablePath = undefined }): Promise<[Browser, Page]> {
    ...
    await context.overridePermissions('https://splice.com', ['clipboard-read']);
...
export async function scrollElemIntoView(page: Page, elem: ElementHandle) {
    for (let i = 0; i < 10; i++) {
        await page.evaluate(elem => elem.scrollIntoView(), elem)
    }
}
